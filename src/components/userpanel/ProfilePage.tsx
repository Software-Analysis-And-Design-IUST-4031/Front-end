...
import settingsAnimation from 'C:/ReactJs/learn/public/lottieflow-menu-nav-11-9-000000-easey.json';
...
interface Post {
  ...
  price: string;
  description: string;
  image: string; // Store Base64-encoded string here
}
...
const ProfilePage: React.FC = () => {
  ...
  // Handle uploading of a new post and display it immediately
  const handleUpload = (data: { title: string; price: string; description: string; image: File }) => {
    const reader = new FileReader();
    reader.onloadend = () => {
      const imageBase64 = reader.result as string; // Convert image to Base64
      const newPost = { ...data, image: imageBase64 };
      const updatedPosts = [...uploadedPosts, newPost];

      setUploadedPosts(updatedPosts);
      localStorage.setItem('uploadedPosts', JSON.stringify(updatedPosts)); // Save to localStorage
    };
    reader.readAsDataURL(data.image); // Trigger Base64 encoding
  };
  ...
  return (
    <Container maxWidth="md">
      ...
      >
        ...
        {/* Upload Icon Button positioned near the bottom */}
